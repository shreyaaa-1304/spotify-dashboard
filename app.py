...
def load_data():
    try:
        ...
        if percentage_cols[0] in df.columns:
            ...
            for col in ['danceability_%', 'valence_%', 'energy_%', 'acousticness_%',
...
if df is not None:
    ...
    with col2:
        # Values are already 0-100; no need to multiply
        avg_danceability = filtered_df['danceability_%'].mean() if 'danceability_%' in filtered_df.columns else 0
        st.metric("Danceability %", f"{avg_danceability:.1f}%", delta="💃")

    with col3:
        avg_energy = filtered_df['energy_%'].mean() if 'energy_%' in filtered_df.columns else 0
        st.metric("Energy %", f"{avg_energy:.1f}%", delta="⚡")

    with col4:
        avg_valence = filtered_df['valence_%'].mean() if 'valence_%' in filtered_df.columns else 0
        st.metric("Valence %", f"{avg_valence:.1f}%", delta="😊")

    st.markdown("---")

    # ── Summary stats heatmap (moved here, after df is loaded) ────────────────
    st.subheader("🔍 Summary Statistics Heatmap")
    summary_cols = [c for c in ["danceability_%", "energy_%", "valence_%", "streams"] if c in filtered_df.columns]
    if summary_cols:
        summary_stats = filtered_df[summary_cols].describe()
        fig_summary = px.imshow(summary_stats, text_auto=True)
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        ...
        "🔮 Linear Regression", "🎯 Clustering", "📈 Detailed Analytics"
    ])
    ...
    with tab1:
        ...
        with col2:
            feature_cols = [c for c in ['danceability_%', 'valence_%', 'energy_%'] if c in filtered_df.columns]
            feature_comparison = filtered_df.groupby('is_hit')[feature_cols].mean()
            feature_comparison.index = ['Not Hit', 'Hit']

            fig_features = go.Figure()
            bar_colors = ['#FF6B6B', '#00D4AA', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8']
            for idx, feature in enumerate(feature_comparison.columns):
    ...
    with tab2:
        ...
        if available_features:
            ...
            for hit_status in [False, True]:
                ...
                # Values are already 0-100; no multiplication needed
                fig_box.add_trace(go.Box(
                    y=filtered_df[filtered_df['is_hit'] == hit_status][selected_feature],
                    name=label,
                    marker_color=color,
                    boxpoints='outliers'
                ))
            ...
            summary_stats = filtered_df.groupby('is_hit')[selected_feature].describe()
    ...
    with tab4:
        ...
        if len(available_features) >= 2:
            ...
            cluster_stats = clustered_df.groupby('cluster')[available_features].mean()
    ...
    with tab5:
        ...
        top_tracks = filtered_df.nlargest(10, 'streams')[
            ['track_name', 'artist(s)_name', 'streams', 'released_year']
        ].copy()
        top_tracks['streams_billion'] = (top_tracks['streams'] / 1e9).round(2)
        st.dataframe(
            top_tracks[['track_name', 'artist(s)_name', 'streams_billion', 'released_year']]
    ...
    st.error("Unable to load the dataset. Please ensure 'spotify_cleaned.csv' is in the correct directory.")
